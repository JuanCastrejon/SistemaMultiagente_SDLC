...
function buildSharedRulesBlock(config) {
  const body = [
    ...
    "7. Jerarquia obligatoria de retrieval (ADR local de jerarquia/perfiles, ver `docs/guides/tool-hierarchy-and-profiles.md`): (0) `Read` directo si path conocido; (1) CodeGraph `codegraph_*` para estructura de codigo; (2) Graphify `graphify query|path|explain` para semantica documental cross-doc; (3) Obsidian vault (`/resume`, lectura targeted) para memoria persistente; (4) OpenSpec `specs/` para capacidades canonizadas; (5) `Grep`/`Glob` solo para texto literal; (6) `WebSearch`/`WebFetch` como ultimo recurso. Saltar a nivel superior MUST estar justificado por respuesta vacia o insuficiente del inferior.",
    "8. Perfiles operativos. Elegir antes de cargar contexto. Documentar `Perfil: <LEAN|ANALYSIS|ORCHESTRATION>` en commit/PR/enhanced. `LEAN` default (CRUD, bug fix, refactor < 3 archivos): CodeGraph + OpenSpec specs minimal + headroom + este bloque; prohibido Graphify, Obsidian retrieval, party-mode, enrich-us, specialist personas. `ANALYSIS` (F2/F3, prior-art, onboarding): `LEAN` + Graphify + Obsidian (`/resume` un checkpoint) + change OpenSpec completo; prohibido party-mode y specialist personas en loop de implementacion. `ORCHESTRATION` (F4 Validacion, tradeoffs, debate multi-voz): `ANALYSIS` + party-mode (<=4 personas, <=3 rondas, <=400 palabras) + enrich-us + specialist personas. Empezar siempre en `LEAN`; upgrade explicito y justificado.",
    "9. Gate local pre-push/pre-PR obligatorio: antes de `git push` o `gh pr create`, ejecutar `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts/validate-local-gate.ps1 -ChangeName <change>` (o script equivalente) y resolver cualquier error de `bootstrap-agent-skills`, `validate:*`, `sdlc governance-check`, `sdlc tools-doctor` y revision `qa-security-review` local."
  ].join("\n");
  return [
